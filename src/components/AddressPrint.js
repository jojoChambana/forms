import { Typography } from "@mui/material";
import { Col, Row } from "react-bootstrap";

export default function AddressPrint({ props, prefix, formValues }) {

    return (

        <>
            <Row>
                {
                    (() => {
                        // if (formValues.formAddress) {
                        if (eval(`formValues.${prefix}ForeignAddressCheckbox`)) {
                            console.log("ForeignAddress")
                            console.log(eval(`formValues.${prefix}ForeignAddress`))
                            return (
                                <>
                                    <Col>
                                        <Typography variant="formCat" className="lineUp">
                                            Address: {eval(`formValues.${prefix}ForeignAddress`)}
                                        </Typography>
                                    </Col>
                                    <Row xs={12} className="resultItems">
                                        <Col>
                                            <Typography variant="formCat" className="lineUp">
                                                City: {eval(`formValues.${prefix}ForeignAddressCity`)}
                                            </Typography>
                                        </Col>
                                    </Row>

                                    <Row xs={12} className="resultItems">
                                        {
                                            (() => {
                                                if (formValues.domesticState) {
                                                    return (
                                                        <>
                                                            <Col>
                                                                <Typography variant="formCat" className="lineUp">
                                                                    Domestic state:
                                                                </Typography>
                                                            </Col>
                                                            <Col className="results">
                                                                <span className="normal">{formValues.domesticState}</span>
                                                            </Col>

                                                            <Col>
                                                                <Typography variant="formCat" className="lineUp">
                                                                    Domestic state:
                                                                </Typography>
                                                            </Col>
                                                            <Col className="results">
                                                                <span className="normal">{formValues.domesticState}</span>
                                                            </Col>
                                                        </>
                                                    );
                                                }
                                            })()
                                        }
                                    </Row>

                                    <Row xs={12} className="resultItems">
                                        {
                                            (() => {
                                                if (formValues.zipCode) {
                                                    return (
                                                        <>
                                                            <Col>
                                                                <Typography variant="formCat" className="lineUp">
                                                                    Zip:
                                                                </Typography>
                                                            </Col>
                                                            <Col className="results">
                                                                <span className="normal">{formValues.zipCode}</span>
                                                            </Col>
                                                        </>
                                                    );
                                                }
                                            })()
                                        }
                                    </Row>
                                </>
                            )
                        } else {
                            console.log("DomesticAddress")
                        }
                    })()
                }
            </Row>

            <Row xs={12} className="resultItems">
                {
                    (() => {
                        if (formValues.domesticCity) {
                            return (
                                <>
                                    <Col>
                                        <Typography variant="formCat" className="lineUp">
                                            Domestic city:
                                        </Typography>
                                    </Col>
                                    <Col className="results">
                                        <span className="normal">{formValues.domesticCity}</span>
                                    </Col>
                                </>
                            );
                        }
                    })()
                }
            </Row>

            <Row xs={12} className="resultItems">
                {
                    (() => {
                        if (formValues.domesticState) {
                            return (
                                <>
                                    <Col>
                                        <Typography variant="formCat" className="lineUp">
                                            Domestic state:
                                        </Typography>
                                    </Col>
                                    <Col className="results">
                                        <span className="normal">{formValues.domesticState}</span>
                                    </Col>
                                </>
                            );
                        }
                    })()
                }
            </Row>

            <Row xs={12} className="resultItems">
                {
                    (() => {
                        if (formValues.zipCode) {
                            return (
                                <>
                                    <Col>
                                        <Typography variant="formCat" className="lineUp">
                                            Zip:
                                        </Typography>
                                    </Col>
                                    <Col className="results">
                                        <span className="normal">{formValues.zipCode}</span>
                                    </Col>
                                </>
                            );
                        }
                    })()
                }
            </Row>
        </>
    );
}
